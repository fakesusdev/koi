...
function import {
    if [ -f "$basedir/Purpur/Purpur-Server/src/main/java/$nms/$1.java" ]; then
...
(
    ...
    if [[ "$lastlog" = *"Koi-Extra mc-dev Imports"* ]]; then
        git reset --hard HEAD^
    fi
)
...
for f in $files; do
    ...
    if [ "$?" == "1" ]; then
        if [ ! -f "$basedir/Purpur/Purpur-Server/src/main/java/net/minecraft/$f" ]; then
...
(
    ...
    rm -rf nms-patches
    git add src -A
    echo -e "Koi-Extra mc-dev Imports\n\n$MODLOG" | git commit src -F -
    exit 0
)
